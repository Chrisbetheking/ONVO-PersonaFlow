...
async function newWorkspaceContext(browser: Browser, workspaceId: string) {
  ...
  await context.addInitScript(
    ...
    { key: workspaceKey, value: workspaceId },
  );
...
test("顾问从机会到审核、跟进、试驾和视频预览的完整路径", async ({
  browser,
}) => {
  ...
  await claim.click();
  await expect(
    page.locator(".trust-item.active").filter({ hasText: "官方" }).first(),
  ).toBeVisible();
  ...
  await expect(page.getByTestId("review-body")).toHaveValue(
    /已根据客户家庭人数准备体验清单/,
  );
  ...
  await page
    .getByTestId("followup-message")
    .fill("周日两点可以到店，希望带儿童推车一起试装。");
  await page.getByTestId("add-followup-message").click();
  await expect(page.getByTestId("conversation-timeline")).toContainText(
    "周日两点可以到店",
  );
  await expect(page.locator('[data-testid^="memory-"]').last()).toContainText(
    "周日两点可以到店",
  );
  ...
  await page
    .getByTestId("booking-notes")
    .fill("先体验满员收纳，再体验周末路线。");
  ...
  await expect(page.getByTestId("video-job-status")).toContainText(
    "仅保存脚本与分镜",
  );
  await expect(page.getByTestId("video-job-status")).toContainText(
    "未生成成片",
...
test("顾问画像保存后切换和刷新仍然存在，重置后恢复", async ({ browser }) => {
  ...
  await page.reload();
  await page.getByTestId("advisor-row-advisor-hz-02").click();
  await expect(page.getByTestId("advisor-audience")).toHaveValue(
    /真实装载体验/,
  );
  ...
  await page.goto("/#/advisors");
  await page.getByTestId("advisor-row-advisor-hz-02").click();
  await expect(page.getByTestId("advisor-audience")).not.toHaveValue(
    /真实装载体验/,
  );
  await context.close();
});
...
test("两个浏览器上下文工作区互不影响，刷新路由不丢失", async ({ browser }) => {
  const contextA = await newWorkspaceContext(
    browser,
    "e2e-isolation-workspace-a",
  );
  const contextB = await newWorkspaceContext(
    browser,
    "e2e-isolation-workspace-b",
  );
  ...
  await expect(
    pageA.getByRole("heading", { name: "内容作战台" }),
  ).toBeVisible();
...
test("网络中断时本地演示仍能走通生成、审核、回复、预约和批量任务", async ({
  browser,
}) => {
  const context = await browser.newContext();
  await context.addInitScript(
    ({ key, value }) => window.localStorage.setItem(key, value),
    { key: workspaceKey, value: "e2e-offline-workspace-001" },
  );
  const page = await context.newPage();
  ...
  await generateContent(page);
  await expect(
    page.getByText(/本地规则演示|离线本地演示/).first(),
  ).toBeVisible();
  ...
  await page
    .getByTestId("followup-message")
    .fill("离线现场：客户确认携带儿童推车到店。");
  await page.getByTestId("add-followup-message").click();
  await expect(
    page.locator('[data-testid^="memory-local-memory-"]').last(),
  ).toContainText("离线现场");
  await page.getByTestId("open-booking").click();
  await page.getByTestId("confirm-booking").click();
  await expect(page.getByTestId("conversation-timeline")).toContainText("周辰");
...
test("顾问画像保存失败时显示错误且不显示成功提示", async ({ browser }) => {
  const context = await newWorkspaceContext(
    browser,
    "e2e-advisor-error-workspace-001",
  );
  const page = await context.newPage();
  await page.route("**/api/advisors/advisor-hz-02", (route) =>
    route.fulfill({
      status: 500,
      contentType: "application/json",
      body: JSON.stringify({ detail: "保存服务不可用" }),
    }),
  );
  await page.goto("/#/advisors");
  await page.getByTestId("advisor-row-advisor-hz-02").click();
...
test("视频接口失败时页面明确显示提交失败", async ({ browser }) => {
  const context = await newWorkspaceContext(
    browser,
    "e2e-video-error-workspace-001",
  );
  const page = await context.newPage();
  await openFirstOpportunity(page);
  await generateContent(page);
  await page.route("**/api/video/start", (route) =>
    route.fulfill({
      status: 502,
      contentType: "application/json",
      body: JSON.stringify({ detail: "视频服务调用失败" }),
    }),
  );
  await page.getByTestId("video-package-toggle").click();
  await page.getByTestId("submit-video-task").click();
...
test("飞书知识变化生成新版本、影响分析和重新核验任务", async ({ browser }) => {
  const context = await newWorkspaceContext(
    browser,
    "e2e-enterprise-knowledge-001",
  );
  const page = await context.newPage();
  await page.goto("/#/knowledge");
  const current = await page
    .locator(".detail-heading .status-pill")
    .innerText();
  ...
  await expect(page.getByTestId("knowledge-impact")).toContainText(
    "待发布内容",
  );
  await expect(page.locator(".detail-heading .status-pill")).not.toHaveText(
    current,
  );
  await context.close();
});
...
test("客户 360 下一最佳行动和承诺台账产生真实状态变化", async ({ browser }) => {
  ...
  await expect(page.getByTestId("promise-promise-xu-route")).toContainText(
    "待执行",
  );
  await page.getByText("演示工具").click();
...
test("员工补充说明后经理创建辅导计划", async ({ browser }) => {
  ...
  await page
    .getByTestId("quality-employee-response")
    .fill("客户引用了旧活动截图，顾问未先核验最新知识版本。");
...
test("角色空间与演示场景状态刷新后保留且 workspace 隔离", async ({
  ...
  const pageB = await contextB.newPage();
  await pageA.goto("/#/today");
  await pageA.getByTestId("space-switcher").click();
...
test("主要企业操作按钮产生状态变化或保持明确禁用", async ({ browser }) => {
  ...
  await page
    .getByTestId("risk-action-dialog")
    .locator("textarea")
    .fill("该客户需要经理协调活动与承诺口径。");
  await page.getByTestId("confirm-risk-action").click();
  await expect(page.getByText("已分配经理").first()).toBeVisible();
...
test("UI 工作台在 1366 宽度无水平溢出且右栏可折叠", async ({ browser }) => {
  const context = await newWorkspaceContext(browser, "e2e-ui-responsive-001");
  const page = await context.newPage();
  ...
  await page.getByRole("button", { name: "收起客户上下文" }).click();
  await expect(page.locator(".customer-context-panel")).toBeHidden();
  await expect(page.getByTestId("followup-message")).toBeVisible();
  await page.getByRole("button", { name: "展开客户上下文" }).click();
  await expect(page.locator(".customer-context-panel")).toBeVisible();
  await openFirstOpportunity(page);
  await generateContent(page);
  await page.getByRole("button", { name: "收起事实与合规侧栏" }).click();
  await expect(page.locator(".trust-panel")).toBeHidden();
  await expect(page.getByTestId("save-draft")).toBeVisible();
  await context.close();
});

test("中文状态与主要操作遵循统一 UI 规则", async ({ browser }) => {
  const context = await newWorkspaceContext(browser, "e2e-ui-consistency-001");
  const page = await context.newPage();
  for (const route of ["today", "customers", "promises"] as const) {
    await page.goto(`/#/${route}`);
    expect(await page.locator("body").innerText()).not.toMatch(
      /\b(?:recommended|pending)\b/,
    );
    const icons = page.locator(".icon-button:visible");
    for (let index = 0; index < (await icons.count()); index += 1)
      expect(await icons.nth(index).getAttribute("aria-label")).toBeTruthy();
  }
  await page.goto("/#/promises");
  await page.getByTestId("promise-promise-xu-route").click();
  expect(
    await page.locator(".promise-action-pane .button-primary:visible").count(),
  ).toBeLessThanOrEqual(1);
  await expect(page.getByText("演示工具")).toBeVisible();
  await context.close();
});

test("内容失效后状态视觉同步且重新核验恢复", async ({ browser }) => {
  const context = await newWorkspaceContext(browser, "e2e-ui-stale-001");
  const page = await context.newPage();
  await openFirstOpportunity(page);
  await generateContent(page);
  const evidenceCard = page.locator(".trust-item").first();
  await expect(evidenceCard).toContainText("已核验");
  const title = page.getByTestId("content-title");
  await title.fill(`${await title.inputValue()} · UI 核验状态测试`);
  await expect(page.getByTestId("content-revalidation-warning")).toBeVisible();
  await expect(evidenceCard).toContainText("待重新核验");
  await expect(evidenceCard).not.toContainText("已核验");
  await expect(page.getByTestId("submit-review")).toBeDisabled();
  await expect(page.getByTestId("mark-content-sent")).toBeDisabled();
  await page.getByTestId("revalidate-content").click();
  await expect(evidenceCard).toContainText("已核验");
  await expect(page.getByTestId("submit-review")).toBeEnabled();
  await context.close();
...
test("刷新后保留当前会话和客户档案对象", async ({ browser }) => {
  const context = await newWorkspaceContext(browser, "e2e-ui-route-object-001");
  const page = await context.newPage();
  await page.goto("/#/followup?customer=customer-xu");
  await expect(page.getByTestId("customer-row-customer-xu")).toHaveClass(
    /active/,
  );
  await page.reload();
  await expect(page).toHaveURL(/customer=customer-xu/);
  await expect(page.getByTestId("customer-row-customer-xu")).toHaveClass(
    /active/,
  );
  await page.goto("/#/customers?customer=customer-xu");
  await expect(page.getByTestId("customer-customer-xu")).toHaveClass(/active/);
  await page.reload();
  await expect(page.getByTestId("customer-customer-xu")).toHaveClass(/active/);
  await context.close();
});
