...
test('中文状态与主要操作遵循统一 UI 规则', async ({ browser }) => {
  ...
  for (const route of ['today', 'customers', 'promises'] as const) {
    await page.goto(`/#/${route}`)
    expect(await page.locator('body').innerText()).not.toMatch(/\b(?:recommended|pending)\b/)
    const icons = page.locator('.icon-button:visible')
    for (let index = 0; index < await icons.count(); index += 1) expect(await icons.nth(index).getAttribute('aria-label')).toBeTruthy()
  }
  await page.goto('/#/promises')
  await page.getByTestId('promise-promise-xu-route').click()
  expect(await page.locator('.promise-action-pane .button-primary:visible').count()).toBeLessThanOrEqual(1)
...
test('刷新后保留当前会话和客户档案对象', async ({ browser }) => {
  ...
  await expect(page.getByTestId('customer-row-customer-xu')).toHaveClass(/active/)
  await page.reload()
