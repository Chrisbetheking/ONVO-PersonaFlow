...
test('顾问从机会到审核、跟进、试驾和视频预览的完整路径', async ({ browser }) => {
  ...
  await expect(page.locator('[data-event-type="test_drive_booked"]').last()).toContainText('周辰')
  await expect(page.locator('[data-event-type="test_drive_booked"]').last()).toContainText('2026-07-20T15:00')
...
test('许先生试驾预约使用实际负责顾问林悦', async ({ browser }) => {
  const context = await newWorkspaceContext(browser, 'e2e-booking-owner-workspace-001')
  const page = await context.newPage()
  await page.goto('/#/followup?customer=customer-xu')
  await page.getByTestId('customer-row-customer-xu').click()
  await expect(page.getByText(/林悦 · 上海浦东体验店/)).toBeVisible()
  await page.getByTestId('open-booking').click()
  await page.getByTestId('booking-time').fill('2026-07-23T19:30')
  await page.getByTestId('booking-items').fill('通勤背包、折叠自行车')
  await page.getByTestId('booking-notes').fill('体验晚高峰城市通勤路线。')
  await page.getByTestId('confirm-booking').click()
  const booking = page.locator('[data-event-type="test_drive_booked"]').last()
  await expect(booking).toContainText('林悦')
  await expect(booking).not.toContainText('周辰')
  await expect(booking).toContainText('2026-07-23T19:30')
  await context.close()
})

test('主要操作产生真实状态变化且可见按钮具备可访问名称', async ({ browser }) => {
  const context = await newWorkspaceContext(browser, 'e2e-button-workspace-001')
  const page = await context.newPage()

  await page.goto('/#/today')
  const opportunity = page.getByTestId('opportunity-row-opp-chen-l80')
  await opportunity.click()
  await page.getByRole('button', { name: /标记完成/ }).click()
  await expect(opportunity).toHaveCount(0)
  await expect(page.getByRole('status')).toContainText('机会已标记完成')

  await page.goto('/#/review')
  await page.getByTestId('review-reason').fill('需要补充动态价格核验时间。')
  await page.getByTestId('return-review').click()
  await expect(page.getByText('已退回').first()).toBeVisible()
  await expect(page.getByText('需要补充动态价格核验时间。')).toBeVisible()

  await page.goto('/#/settings')
  await page.getByRole('button', { name: '执行事实与风险预检' }).click()
  await expect(page.getByText('禁用或高风险表达').first()).toBeVisible()
