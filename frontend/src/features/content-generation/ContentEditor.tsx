...
export function ContentEditor({
  ...
  return (
    <div className="editor-workspace">
      ...
      <label className="field-label">标题<input value={variant.title} onChange={event => onChange({ title: event.target.value })} /></label>
      <label className="field-label editor-body-label">正文<textarea value={variant.body} onChange={event => onChange({ body: event.target.value })} /></label>
      ...
      <label className="field-label">行动引导<input value={variant.call_to_action} onChange={event => onChange({ call_to_action: event.target.value })} /></label>
      ...
      <div className="annotated-preview">
        ...
        <p>{segments.map((segment, index) => segment.type === 'plain' ? <span key={index}>{segment.text}</span> : <button key={index} className={`inline-mark inline-${segment.type}`} onClick={() => segment.type === 'claim' ? onSelectEvidence(segment.refId || '') : onSelectRisk(segment.refId || '')}>{segment.text}</button>)}</p>
      </div>

      <div className="editor-actions">
        <Button variant="ghost" onClick={onRegenerate}><RotateCcw size={16} />重新生成此版本</Button>
        <div><Button variant="secondary" loading={saving} onClick={onSave}><Save size={16} />保存草稿</Button><Button loading={submitting} disabled={!canSubmit} onClick={onSubmit}><Send size={16} />提交审核</Button></div>
      </div>
