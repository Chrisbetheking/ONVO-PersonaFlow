          ...
          {selected ? <article className="review-detail" data-testid="review-detail">
            ...
            <div className="review-content-grid">
              <section className="review-content-editor">
                <label className="field-label">完整正文<textarea data-testid="review-body" value={body} onChange={event => setBody(event.target.value)} /></label>
                <label className="field-label">行动引导<input data-testid="review-cta" value={callToAction} onChange={event => setCallToAction(event.target.value)} /></label>
            ...
            <div className="review-actions"><Button data-testid="return-review" variant="danger" loading={working} onClick={() => void decide('returned')}><XCircle size={16} />退回修改</Button><Button data-testid="approve-review" loading={working} onClick={() => void decide('approved')}><CheckCircle2 size={16} />批准发布</Button></div>
