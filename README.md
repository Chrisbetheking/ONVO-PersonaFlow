...
ONVO PersonaFlow v0.4.1 延续 v0.3.x 的今日机会、内容作战台、跟进、审核和批量任务，在同一 React / TypeScript / Vite + FastAPI 工程中增加企业知识、热点雷达、客户 360、承诺台账、质量辅导、优秀案例和 Demo Adapter。它不是自动处罚系统，也不把演示数据伪装成生产数据。
...
- 标题、正文、行动引导、局部改写和合规建议修改都会使当前版本变为“待重新核验”；
- 右侧事实证据同步变为灰黄警告状态，不再与顶部失效提示矛盾；
- 服务端使用签名核验凭证阻止客户端伪造“已核验”、绕过送审或发送；
- 经理修改审核内容后同样必须重新核验；
- 客户承诺的负责顾问由服务端根据客户档案确定，不接受客户端覆盖；
- 顾问、承诺、审核、跟进、知识同步、风险处理和场景切换写入当前 workspace 审计记录。
